...
def cli(ctx, data_type, input_file, out, search_range, processes_number) -> None:
    ...
    island_annotations = data_processor.load_islands()
    visualization = IslandAnalyzer(cpgs=selected, mynorm=data_processor.mynorm, 
                                  manifest=data_processor.manifest, 
                                  islands_annotations=island_annotations)
